// Exa's actual logo mark + wordmark, extracted from exa.ai (viewBox 0 0 278 100)
// and their public brand page (exa.ai/brand), which lists #1F40ED as the mark's
// blue. The wordmark glyphs use currentColor so they follow light/dark theme;
// the blue mark stays fixed since it's the one color Exa uses in both themes.

export const EXA_BLUE = "#1F40ED";

/** Just the geometric "X" mark, no wordmark — for compact placements. */
export function ExaLogoMark({ className }: { className?: string }) {
  return (
    <svg
      viewBox="0 0 78.1818 100"
      className={className}
...
/** Full "[mark] exa" lockup — wordmark inherits text color from its parent. */
export function ExaLogoFull({ className }: { className?: string }) {
  return (
    <svg
      height="20"
      viewBox="0 0 278 100"
